...
class Model:
    def __init__(
        ...
        if phase == Phase.Validation:
            ...
            correct = tf.equal(true_labels, predictions)
            correct = tf.cast(correct, tf.float32)
            self._accuracy = tf.reduce_mean(correct)

            self._TP =  tf.count_nonzero(predictions*true_labels, dtype=tf.float32)
            self._TN = tf.count_nonzero((true_labels - 1) * (predictions - 1), dtype=tf.float32)
            self._FP = tf.count_nonzero(predictions * (true_labels - 1), dtype=tf.float32)
            self._FN = tf.count_nonzero((predictions - 1) * true_labels, dtype=tf.float32)
